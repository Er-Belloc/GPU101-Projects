...
enum class directions : unsigned int // Works faster than a char
{
	invalid = 0,
	match,
	mismatch,
...
int main(int argc, char* argv[])
{
...
#ifdef CHECK_COORDS
	printf("Warning: Compiled with -D CHECK_COORDS\n");
#endif
...
#else
	CPU_res = (int*)malloc(NUM * 3 * sizeof(int));
	GPU_res = (int*)malloc(NUM * 3 * sizeof(int));
#endif
	...
	const bool ok2 = GPU_ComputeSW(NUM, SEQ_LEN, queries, references, GPU_simple_rev_cigar, GPU_res, GPU_millis);
...
#else
		for (unsigned i = 0; i < NUM * 3 && good; ++i)
			good = (CPU_res[i] == GPU_res[i]);

		if (good)
			printf("Max scores and coordinates correct\n");
		else
			printf("Max scores or coordinates wrong\n");
#endif
...
#else
		CPU_res[3 * n] = scores[max_i * matrix_col + max_j];
		CPU_res[3 * n + 1] = max_i;
		CPU_res[3 * n + 2] = max_j;
#endif
	...
	char* GPU_simple_rev_cigar, int* GPU_res, float& millis)
{
	...
	const auto initSW = [&]()
	{
		const dim3 threadsPerBlock // Once (16, 16, 4)
		{
			(unsigned)4, //(dim_seqs / prop.warpSize),
			(unsigned)4, //(dim_seqs / prop.warpSize),
			(unsigned)1// (prop.maxThreadsPerBlock / (dim_seqs / prop.warpSize * dim_seqs / prop.warpSize))
		};

		const dim3 blocksPerGrid // Once (1, 1, 250)
		{
			(unsigned)1,
			(unsigned)1,
	...
	{
		const dim3 threadsPerBlock // Once (32, 1, 8)
		{
			(unsigned)prop.warpSize,
			(unsigned)1,
			(unsigned)1 //(THREADS_PER_BLOCK / prop.warpSize) // Keep it low since these threads needs to be synchronized
		};
...
#else
	CHECK_CUDA(cudaMalloc((void**)&dev_res, num_entries * 3 * sizeof(dev_res[0])));
#endif
...
#else
	CHECK_CUDA(cudaMemcpy(GPU_res, dev_res, num_entries * 3 * sizeof(dev_res[0]), cudaMemcpyKind::cudaMemcpyDeviceToHost));
#endif
	...
	)
{
	...
	for (unsigned i = x + 1; i < row; i += blockDim.x) // Never goes outside the matrix, do not process first row
	{
		for (int j = start_j; j < stop_j; j++) // Might access memory before and after each row
		{
			...
			if (valid) // Reduces accesses to memory
			{
				...
				// Find max score		
				const bool mux_match	= upleft > 0; //  0 > upleft;
				...
				dev_entries[index_this].score = out_score; //valid * out_score + !valid * dev_entries[index_this].score; // max(max1, max2)
				dev_entries[index_this].dir = out_dir; //directions(valid * (unsigned)out_dir + !valid * (unsigned)dev_entries[index_this].dir);

				const bool is_greater = valid && out_score > max_score;
				max_score = is_greater * out_score + !is_greater * max_score;
...
#else
		dev_res[3 * z] = shared_memory[threadMaxIndex];
		dev_res[3 * z + 1] = shared_memory[threadMaxIndex + 1];
		dev_res[3 * z + 2] = shared_memory[threadMaxIndex + 2];
#endif
